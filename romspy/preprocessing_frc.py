...
# Class for creating forcing files:
class PreProcessorFrc:
    def __init__(self, ROMSsetup: str, outdir: str, target_grid: str, sources: list, **kwargs):
        ...
        if self.verbose > 0:
            print("Finished PreProcessorFrc setup")
    ...
    def make(self):
        ...
        print("PreProcessorFrc --> make called")
        sys.stdout.flush()
        ...
        for group, group_index in zip(self.sources, count()):
            ...
            if group['data_source'] == 'ERA5':
                ...
                for yr in range(yr1,yr2+1):
                    for m in range(1,13):
                        fname = '{0}/{1}/ERA5_{1}_{2:02}.nc'.format(dfolder,yr,m)
                        if self.verbose > 1:
                            print('ERA5 fname = '+fname)
                ...
                #print('interpolator.make: file list: {}'.format(group_files))
            elif 'files' in group:
            ...
            if 'files' in group:
                ...
                for in_file, file_index in zip(group['files'], fidx_iter):
                    ...
                    interp.interpolate(in_file, out_file, group, group_index, variables, group_files)
    ...
    def make_adjustments(self, file: str, out_variables: set, group_index: int, group_files: str, all_vars: set):
        ...
        if dsource == 'ERA5':
            if tres == '1d':
                ...
                wind_stress_scale = 1/3600.0
                # Solar flux conversion factor (to convert from J/m^2 to W/m^2):
                swrad_scale = 1/3600.0
            else:
                msg = f'time resolution not supported: {tres}\n'
        ...
        for adjustment in self.adjustments:
            ...
            if len(adjustment['out_var_names'] - all_vars) > 0 or len(adjustment['out_var_names']) == 0:
                ...
                if len(adjustment['in_var_names'] & out_variables) == len(adjustment['in_var_names']):
                    ...
                    if self.verbose > 0:
                        adj_name = str(adjustment['func'].__name__)
                        print("Calling adjustment: " + adj_name)
                        sys.stdout.flush()
                        # import pdb
                        t1 = time.time()
                        ...
                        t2 = time.time()
                        print(f"time for adjustment {adj_name}: {t2-t1:.1f} s")
                        sys.stdout.flush()
